...
pub const enable = if (builtin.is_test) false else config.enable_tracy;
...
pub const callstack_depth = if (enable_callstack and config.tracy_callstack_depth > 0) config.tracy_callstack_depth else 10;
